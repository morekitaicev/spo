...
o = open('election_data.csv', 'w')
u = open('beglov_data.csv', 'w')
...
for j in range(31):
    ...
    str_TIK = 'TIK ' + str(j+1)
    yavka_TIK = str((big_table[2,j] + big_table[3,j]) / big_table[0,j])
    beglov_TIK = str(big_table[12,j] / (big_table[2,j] + big_table[3,j]))
    if j < 30:
        o.write(str_TIK + ';' + yavka_TIK + '\n')
        u.write(str_TIK + ';' + beglov_TIK + '\n')
    plt.title(str_TIK)
